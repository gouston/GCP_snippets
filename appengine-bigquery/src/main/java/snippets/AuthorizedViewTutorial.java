...
public class AuthorizedViewTutorial {
  ...
  public static void main(String[] args) {
    ...
    for (String destViewId : destViewIds) {
      ...
      authorizeViewAcrossProjects(srcProjectId,
          destProjectId, sourceDatasetId, destDatasetId, destViewId);

    }

  }

  public static void authorizeViewAcrossProjects(
      String srcProjectId,
      ...
      String destViewId) {
    try {
      // Initialize source client that will be used to send requests.
      // This client only needs to be created
      // once, and can be reused for multiple requests.
      BigQuery srcBigquery = BigQueryOptions.newBuilder().setProjectId(srcProjectId).build().getService();

      // Get the source dataset where your tables are kept
      final Dataset sourceDataset = srcBigquery.getDataset(sourceDatasetId);

      // Authorize the view to access the source dataset
      List<Acl> srcAcl = new ArrayList<>(sourceDataset.getAcl());
      srcAcl.add(Acl.of(new Acl.View(TableId.of(destProjectId, destDatasetId, destViewId))));
  ...
  public static void sameProject() {
  ...
  public static void authorizedViewTutorial(
    ...
    try {
      ...
      String tableQuery = "SELECT commit, author, committer, repo_name"
          + " FROM `bigquery-public-data.github_repos.commits`"
          + " LIMIT 1000";
      QueryJobConfiguration queryConfig = QueryJobConfiguration.newBuilder(tableQuery)
      ...
      String viewQuery = String.format(
          "SELECT commit, author.name as author, "
              + "committer.name as committer, repo_name FROM %s.%s.%s",
          projectId, sourceDatasetId, sourceTableId);
      ViewDefinition viewDefinition = ViewDefinition.of(viewQuery);
      Table view = bigquery.create(TableInfo.of(TableId.of(sharedDatasetId, sharedViewId), viewDefinition));
